...
Plan standalone — turns an Agent Vision (or plain-English description) into a structured eval plan grounded in the 10-step playbook: eval objective, risk tier, owner, capability and trust & safety eval sets, Value × Risk prioritization, pass/fail conditions, pass-rate targets, gates, methods, and human inputs. Output is a customer-ready `.docx` eval plan. Use before generating test cases or running any evals.
